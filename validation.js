...
function employee_validation(e) {
  ...
  if (!usernamePattern.test(username)) {
    alert(
      "Username must be 3-50 letters (A-Z, a-z) and may optionally end with a number"
    );
    document.getElementById("e_username").focus();
    return false;
  }
...
function customer_validation(e) {
  ...
  if (!/^[0-9]{10}$/.test(phone)) {
    alert("Phone must be digits only (10 digits)");
    return false;
  }
